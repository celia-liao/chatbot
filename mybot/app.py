...
def generate_fortune_card(pet_id: int) -> str:
    ...
    try:
        ...
        # ===== 寵物頭像位置調整參數 =====
        y_offset = 250  # 垂直位置（240 = 原本位置，正數 = 往下移，負數 = 往上移）
        # ===== 調整說明 =====
        # - 往下移動 10 像素：y_offset = 250
        # - 往上移動 10 像素：y_offset = 230
        # - 目前設定：250（往下移動 10 像素）
        ...
        # ===== 覆蓋圖（cover_image）位置調整參數 =====
        cover_x = 0  # 覆蓋圖水平位置（0 = 最左邊，正數 = 往右移，負數 = 往左移）
        cover_y = 0  # 覆蓋圖垂直位置（0 = 最上邊，正數 = 往下移，負數 = 往上移）
        # ===== 調整說明 =====
        # - 如果覆蓋圖需要往右移動 10 像素：cover_x = 10
        # - 如果覆蓋圖需要往下移動 20 像素：cover_y = 20
        # - 如果覆蓋圖需要往左移動 5 像素：cover_x = -5
        # - 如果覆蓋圖需要往上移動 15 像素：cover_y = -15
